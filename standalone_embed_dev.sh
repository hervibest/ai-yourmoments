...
start() {
    # Cek apakah sudah running dan sehat
    res=$(sudo docker ps | grep milvus-standalone | grep healthy | wc -l)
    if [ "$res" -eq 1 ]; then
    ...
    # Cek apakah container exist dengan docker inspect (bukan grep)
    if sudo docker container inspect milvus-standalone > /dev/null 2>&1; then
        sudo docker start milvus-standalone > /dev/null
    else
        run_embed
    fi
